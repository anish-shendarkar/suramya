...
import { WomensCategorySidebar } from "@/components/WomenSidebar";
...
export default function CategoryPage() {
    ...
    const fetchOutfits = async (page: number) => {
        try {
            const response = await fetch(`http://localhost:3333/user/outfits/female?page=${page}&limit=12`, {
                method: "GET",
            });
            if (!response.ok) {
                ...
                <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
                    {filteredOutfits.length > 0 ? (
                        filteredOutfits.map((outfit) => (
                            <div
                                onClick={() => handleOnClick(outfit._id)}
                                key={outfit?._id}
                                className="rounded-lg p-3 sm:p-4 shadow-md border-2 border-transparent transition-all duration-300 hover:border-rose-300 hover:shadow-purple-400 cursor-pointer bg-white"
                            >
                                ...
                                <h2 className="text-base sm:text-xl font-semibold mt-2 line-clamp-1">
                                    {outfit?.name}
                                </h2>
